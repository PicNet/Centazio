...
public abstract class AbstractEFCtlRepository(Func<AbstractCtlRepositoryDbContext> getdb) : AbstractCtlRepository {
  ...
  private AbstractCtlRepositoryDbContext? running;
  protected Func<AbstractCtlRepositoryDbContext> Db => () => running ?? getdb();

  public override async Task<IDbTransactionWrapper> BeginTransaction() {
    await using var db = Db();
    running = db;
    var impl = await db.Database.BeginTransactionAsync();
    return new EfTransactionWrapper(impl, () => running = null);
  }
