...
public abstract class AbstractEFCtlRepository(Func<AbstractCtlRepositoryDbContext> getdb) : AbstractCtlRepository {
  ...
  protected async Task CreateSchema(AbstractCtlRepositoryDbContext db) =>
      await db.CreateDb();

  protected async Task DropTablesImpl(AbstractCtlRepositoryDbContext db) { 
    await db.DropDb(
        new (db.Settings.SchemaName, db.Settings.CoreToSysMapTableName), 
        new (db.Settings.SchemaName, db.Settings.ObjectStateTableName), 
        new (db.Settings.SchemaName, db.Settings.SystemStateTableName), 
        new (db.Settings.SchemaName, db.Settings.EntityChangeTableName));
  }
}