...
public static class ReflectionUtils {
  ...
  public static List<Assembly> GetProviderAssemblies() {
    return new DirectoryInfo(FsUtils.GetCliPath()).GetFiles("*.dll", SearchOption.AllDirectories)
  ...
  public static string GetAssemblyPath(string assembly) {
    ...
    var dlls = Directory.GetFiles(FsUtils.GetCliPath(), "*.dll", SearchOption.AllDirectories).Where(dll => dll.EndsWith(fname)).ToList();
    ...
    return assfile ?? throw new FileNotFoundException($"File for assembly [{assembly}] could not be found in directory (recursively) [{FsUtils.GetCliPath()}]");
  }
