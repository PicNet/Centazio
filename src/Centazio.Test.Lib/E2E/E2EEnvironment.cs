...
public class E2EEnvironment(ISimulationProvider provider) : IAsyncDisposable {
  ...
  private async Task CompareMembershipTypes() {
    var core_types = (await ctx.CoreStore.GetMembershipTypes(m => true)).Select(m => ctx.Converter.CoreMembershipTypeToCrmMembershipType(Guid.Empty, m));
  ...
  private async Task CompareCustomers() {
    var core_customers_for_crm = (await ctx.CoreStore.GetCustomers(m => true)).Select(c => ctx.Converter.CoreCustomerToCrmCustomer(Guid.Empty, c));
    var core_customers_for_fin = (await ctx.CoreStore.GetCustomers(m => true)).Select(c => ctx.Converter.CoreCustomerToFinAccount(0, c));
  ...
  private async Task CompareInvoices() {
    var cores = await ctx.CoreStore.GetInvoices(m => true);
