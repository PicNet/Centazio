...
public class AbstractFunctionStaticHelperTests {
  ...
  [Test] public void Test_GetReadyOperations_correctly_filters_out_operations_not_meeting_cron_criteria() {
    OperationStateAndConfig<ReadOperationConfig> Op(string name, string cron, DateTime last) => new(
        new ObjectState(name, name, new ExternalEntityType(name), true) { 
          LastCompleted = last,
  ...
  [Test] public async Task Test_RunOperationsTillAbort_stops_on_first_exception() {
    ...
    states[0] = states[0] with { OpConfig = states[0].OpConfig with { GetObjectsToStage = new ErrorReadOperationImplementation() } }; 
  ...
  private ObjectState ExpObjState(EOperationResult res, EOperationAbortVote vote, int len, string exmessage="na") {
    return new ObjectState(res.ToString(), res.ToString(), new ExternalEntityType(res.ToString()), true) {
  ...
  private class ErrorReadOperationImplementation : IGetObjectsToStage {
    ...
    public Task<ReadOperationResult> GetUpdatesAfterCheckpoint(OperationStateAndConfig<ReadOperationConfig> config) => throw new Exception();
