...
public abstract class AbstractCoreToSystemMapStoreTests {
  ...
  [Test] public async Task Test_upsert_single() {
    ...
    var created = (await entitymap.Create(Constants.CoreEntityName, Constants.System1Name, [original.SuccessCreate(STR, Guid.NewGuid().ToString())])).Single();
  ...
  [Test] public async Task Test_upsert_enum() {
    var original = new List<CoreToExternalMap.Created> { 
      CoreToExternalMap.Create(TestingFactories.NewCoreCust(STR, STR), STR).SuccessCreate(STR, Guid.NewGuid().ToString()),
      CoreToExternalMap.Create(TestingFactories.NewCoreCust(STR2, STR2), STR2).SuccessCreate(STR2, Guid.NewGuid().ToString())
    }; 
  ...
  [Test] public async Task Test_duplicate_mappings_found_in_simulation() {
    ...
    await entitymap.Create(Constants.CoreEntityName, Constants.System1Name, gfc1.Created.Select(c => c.Created("71c5db4e-971a-45f5-831e-643d6ca77b20", Guid.NewGuid().ToString()).Map).ToList());
    ...
    var ex = Assert.ThrowsAsync<Exception>(() => entitymap.Create(Constants.CoreEntityName, Constants.System1Name, gfc2.Created.Select(c => c.Created("71c5db4e-971a-45f5-831e-643d6ca77b20", Guid.NewGuid().ToString()).Map).ToList()));
  ...
  [Test] public async Task Reproduce_duplicate_mappings_found_in_simulation() {
    ...
    async Task<CoreEntity> SimulatePromoteOperationRunner(string coreid, SystemName system, string externalid) {
      ...
      await corestore.Upsert(Constants.CoreEntityName, [new Containers.CoreChecksum(c, Helpers.TestingChecksum(c))]);
      await entitymap.Create(Constants.CoreEntityName, system, [CoreToExternalMap.Create(c, system).SuccessCreate(externalid, Guid.NewGuid().ToString())]);
      return c;
    }
    ...
    await entitymap.Create(Constants.CoreEntityName, Constants.System1Name, [CoreToExternalMap.Create(c1, Constants.System2Name).SuccessCreate("E2", Guid.NewGuid().ToString())]);
