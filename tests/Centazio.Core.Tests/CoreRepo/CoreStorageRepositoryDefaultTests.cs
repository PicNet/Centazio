...
public abstract class CoreStorageRepositoryDefaultTests {

  private ICoreStorageRepository repo;
  ...
  protected abstract ICoreStorageRepository GetRepo();
  ...
  [Test] public async Task Test_insert_get_update_get() {
    ...
    var list1 = (await repo.Query<CoreCustomer>(e => true)).ToList();
    var updated = retreived1 with { FirstName = "N2" };
    await repo.Upsert(updated);
    var retreived2 = await repo.Get<CoreCustomer>(id);
    var list2 = (await repo.Query<CoreCustomer>(e => true)).ToList();
  ...
  [Test] public async Task Test_query() {
    var data = Enumerable.Range(0, 100).Select(idx => new CoreCustomer(Guid.NewGuid().ToString(), $"N{idx}", $"N{idx}", DateOnly.FromDateTime(TestingDefaults.DefaultStartDt.AddDays(idx)), UtcDate.UtcNow)).ToList();
    await repo.Upsert(data);
    
    var all = (await repo.Query<CoreCustomer>(e => true)).ToList();
    var even = (await repo.Query<CoreCustomer>(e => Int32.Parse(e.FirstName.Substring(1)) % 2 == 0)).ToList();
    var odd = (await repo.Query<CoreCustomer>(e => Int32.Parse(e.FirstName.Substring(1)) % 2 == 1)).ToList();
...
public class InMemoryCoreStorageRepositoryTests : CoreStorageRepositoryDefaultTests {

  protected override ICoreStorageRepository GetRepo() => new InMemoryCoreStorageRepository();
