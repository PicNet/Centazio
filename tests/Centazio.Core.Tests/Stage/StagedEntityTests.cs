...
public class StagedEntityTests {
  ...
  [Test] public void Test_initialisation_handles_ignore_correctly() {
    Assert.That(new StagedEntity(Guid.CreateVersion7(), NAME, NAME, UtcDate.UtcNow, NAME, TestingFactories.TestingChecksum(NAME), Ignore: null).Ignore, Is.Null);
    Assert.That(new StagedEntity(Guid.CreateVersion7(), NAME, NAME, UtcDate.UtcNow, NAME, TestingFactories.TestingChecksum(NAME), Ignore: "").Ignore, Is.Null);
    Assert.That(new StagedEntity(Guid.CreateVersion7(), NAME, NAME, UtcDate.UtcNow, NAME, TestingFactories.TestingChecksum(NAME), Ignore: " ").Ignore, Is.Null);
    Assert.That(new StagedEntity(Guid.CreateVersion7(), NAME, NAME, UtcDate.UtcNow, NAME, TestingFactories.TestingChecksum(NAME), Ignore: "\n\t ").Ignore, Is.Null);
  }
  ...
  private record SubClassSE(string? Ignore = null) : StagedEntity(Guid.CreateVersion7(), NAME, NAME, UtcDate.UtcNow, NAME, TestingFactories.TestingChecksum(NAME), Ignore: Ignore);
}