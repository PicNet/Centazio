...
public class EntityStagerTests {
  ...
  [Test] public async Task Test_staging_a_single_record() {
    ...
    var results1 = await stager.GetUnpromoted(UtcDate.UtcNow.AddMilliseconds(-1), NAME, NAME);
    var results2 = await stager.GetUnpromoted(UtcDate.UtcNow, NAME, NAME);
  ...
  [Test] public async Task Test_staging_a_multiple_records() {
    ...
    var results1 = await stager.GetUnpromoted(UtcDate.UtcNow.AddMicroseconds(-1), NAME, NAME);
    var results2 = await stager.GetUnpromoted(UtcDate.UtcNow, NAME, NAME);
