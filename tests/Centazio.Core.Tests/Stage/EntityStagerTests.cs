...
public class EntityStagerTests {
  ...
  [Test] public async Task Test_staging_a_single_record() {
    await stager.Stage(UtcDate.UtcNow, NAME, NAME, nameof(EntityStagerTests));
    
    var results1 = await stager.Get(UtcDate.UtcNow.AddMilliseconds(-1), NAME, NAME);
    var results2 = await stager.Get(UtcDate.UtcNow, NAME, NAME);
    
    var staged = results1.Single();
    Assert.That(staged, Is.EqualTo(new StagedEntity(NAME, NAME, UtcDate.UtcNow, nameof(EntityStagerTests))));
  ...
  [Test] public async Task Test_staging_a_multiple_records() {
    ...
    await stager.Stage(UtcDate.UtcNow, NAME, NAME, datas);
    
    var results1 = await stager.Get(UtcDate.UtcNow.AddMicroseconds(-1), NAME, NAME);
    var results2 = await stager.Get(UtcDate.UtcNow, NAME, NAME);
    
    var exp = Enumerable.Range(0, 10).Select(i => i.ToString()).Select(idx => new StagedEntity(NAME, NAME, UtcDate.UtcNow, idx.ToString())).ToList();
