...
SETTING7=val with # should not ignore";
  ...
  [Test] public async Task Test_loading_from_local() {
    Assert.That(await Load(("testing", FULL_CONTENT)), Is.EqualTo(new TestSettingsTargetObj("VALUE1;", "VALUE 2 with spaces", 123, "trailing space with semmi ;", "val;with;semmis;", "val=with=equals", "val with # should not ignore")));
  }
  
  [Test] public async Task Test_overwriting_secrets() {
    Assert.That(await Load(("testing", FULL_CONTENT), ("overwrite", "SETTING4=overwritten")), Is.EqualTo(new TestSettingsTargetObj("VALUE1;", "VALUE 2 with spaces", 123, "overwritten", "val;with;semmis;", "val=with=equals", "val with # should not ignore")));
  }
  ...
  private record TestSettingsTargetObj(string SETTING1, string SETTING2, int SETTING3_NUMBER, string SETTING4, string SETTING5, string SETTING6, string SETTING7);
}