...
public class CheckCorrectUseageOfRecordHierarchies {
  ...
  [Test] public void Test_record_hierarchies() {
    ...
    var records = InspectUtils.LoadCentazioAssemblies()
        ...
        .ToList();
    var resultbases = records.Where(r => r.IsAbstract && r.Name.Contains("Result", StringComparison.Ordinal)).ToList();
    ...
    resultbases.ForEach(baserec => {
      ...
      if (impls.Any()) errors.Add($"abstract 'Result' records should not export any subclasses, i.e. they should be 'internal sealed' and be created using factory methods on the base abstract record.");
    });
