...
public class TestingEfCtlRepository(Func<AbstractCtlRepositoryDbContext> getdb, IDbFieldsHelper dbf) : AbstractEFCtlRepository(getdb), ITestingCtlRepository {
  ...
  public override async Task<ICtlRepository> Initialise() => 
      await UseDb(async db => {
        // note: EnsureDeletedAsync / EnsureCreatedAsync is very slow when compared to  DropTablesImpl / CreateSchema
        //    also EnsureDeletedAsync / EnsureCreatedAsync require that master is not used in SqlConn, this causes other
        //    problems
